...
const InfoBox: React.FC<{ className?: string; children: string }> = ({
  className,
  children,
}) => {
      ...
      {(!infoOpen && (
        <div className="w-full flex flex-row justify-end">
          <button
            ...
            className={t(
              infoOpen ? "" : "rounded-b-md shadow-md",
              "block p-1 bg-slate-800 border border-slate-600 w-10 h-10 outline-slate-600 outline-1 -outline-offset-4 rounded-t-md active:shadow-inner select-none",
            )}
          >
        ...
        <Overlay title="Help Guide" onClose={() => setInfoOpen(false)}>
          <ul>
            ...
            <InfoRow icon={<Search className="w-5" />} border>
              filters todos by text and allows for exact match with double
              quotes
            </InfoRow>
  ...
  return (
    <li
      className={t(
        ...
        "text-sm py-1 flex flex-row gap-1",
      )}
    >
      {icon && icon}
